...
int main(int argc,char* argv[])
{
    ...
    //traverse the vector and erase targets
    for (int i = 0; i < diaries.size();)
    {
        if ((yearSpecified && diaries[i].year == y && !monthSpecified) || ((yearSpecified && diaries[i].year == y) && (monthSpecified && diaries[i].month == m) && !daySpecified) || ((yearSpecified && diaries[i].year == y) && (monthSpecified && diaries[i].month == y) && (daySpecified && diaries[i].day == y)))
        {
    ...
    ofstream fout(PD_FILE_NAME);    //open out file stream
    //output diaries into file
    pdNum = diaries.size();
    fout << pdNum << "\n";
    for (int i = 0; i < pdNum; i++)
    {
    ...
    fout.close();   //close out file stream
    return somethingErased ? 0 : -1;
}